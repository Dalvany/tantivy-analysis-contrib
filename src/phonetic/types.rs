...
pub struct MaxCodeLength(pub(crate) Option<usize>);
...
pub struct DMRule(pub(crate) String);
...
#[derive(Copy, Clone, Debug, Ord, PartialOrd, Eq, PartialEq, Hash)]
pub struct Mapping(pub(crate) Option<[char; 26]>);

/// Indicate, for Soundex, if `H` and `W` should be treated as silence.
///
/// Default to `true`.
#[derive(Copy, Clone, Debug, Ord, PartialOrd, Eq, PartialEq, Hash)]
pub struct SpecialHW(pub(crate) Option<bool>);
